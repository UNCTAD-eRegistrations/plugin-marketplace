...
root [`kimi-marketplace.json`](./kimi-marketplace.json) is a Kimi marketplace catalog whose
plugin sources are per-plugin zips on the rolling
[`kimi-latest`](https://github.com/UNCTAD-eRegistrations/plugin-marketplace/releases/tag/kimi-latest)
release (rebuilt on every push to `main` by the `publish-kimi-plugins` workflow).

In Kimi Code:

```
/plugins marketplace https://raw.githubusercontent.com/UNCTAD-eRegistrations/plugin-marketplace/main/kimi-marketplace.json
/plugins install bpa-mcp        # install this one first — all others depend on it
/reload
```

Installing a single GitHub repo URL (`/plugins install <repo-url>`) does **not** work for
...
```
/plugins marketplace ./plugin-marketplace/kimi-marketplace.local.json
