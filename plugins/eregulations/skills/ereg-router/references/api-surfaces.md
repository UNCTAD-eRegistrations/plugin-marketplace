...
Coolify "Domains" format: `https://<host>:<internal port>`. No path prefixes anywhere. A release walks three stages (`docs/RELEASING.md`, `releases/channels.yml`): the intent is `releases/<version>.yml` on `main`; the bot builds it on a `release/<version>` branch and a GitHub Release `v<version>`; then it is promoted ring by ring — `channel/next` (auto-merged once its checks pass, no soak) and `channel/stable` (merged by the release manager after a 24-hour soak on next, 0 for a hotfix; the production instances subscribe here). Each ring branch carries one `release.yml` (format 2) that pins Public, Admin, Statistics and SPA to `main` by SHA and records every image's tag and digest — read the ring the instance subscribes to rather than guessing which branch an image came from. On 2026-09-11 both rings serve 7.4.2. A Coolify app subscribes to a ring by pointing at its branch with auto-deploy on; `channel/<major.minor>` line branches appear when a new minor reaches stable. `docker-compose.yml` on `main` references the `:unreleased` tags. Instance lifecycle and creation are in `docs/INSTANCE-LIFECYCLE.md` and `docs/NEW-INSTANCE.md`; the `eregulations migrate` CLI (eight resumable phases) is in the repo since 2026-09-08 although `NEW-INSTANCE.md` still calls it planned.
...
| eRegulations-deploy | `channel/next` | `ea92502` | 2026-09-11 | `release.yml` = 7.4.2, same four SHAs as stable (the ring a release reaches first) |
