...
| `references/api-surfaces.md` | the three HTTP API surfaces per version line: how to identify which one you face, per-version contracts (auth, language, casing), endpoint indexes, recipes, migration notes, known defects (a code-derived hint, not a source of truth) |
