...
4. **The shared document holds only what is agreed (Frank, 23-08-2026).** The current situation never moves: it is read from the live service. A colleague who wants a different end situation draws it with this same kit, and the drawing lands in HIS part, never in the main body: either the document's per-colleague section (numbered JP1, JP2, dated, with the part it answers) or, where a section carries the split view, its "Agreed contract · <name>'s try" tabs. The agreed pane is always the default view. A try moves into the contract only when both agree, and the row records the move. Remarks follow the same law in their own section (J1, J2, four states, nothing deleted).
