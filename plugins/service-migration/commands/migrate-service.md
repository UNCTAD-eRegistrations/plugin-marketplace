...
argument-hint: <service-id> <source-server> <target-server> [--dry-run]
...
- Second: source MCP server name (e.g., `BPA-jamaica`)
- Third: target MCP server name (e.g., `BPA-lesotho2`)
...
1. **Export** full service from source: `mcp__<source>__service_export_raw`
...
6. **Create** service on target via `mcp__<target>__service_copy` or component-by-component
7. **Verify** by running `mcp__<target>__debug_scan` on the new service
8. **Report**: new service ID on target, list of components that need manual follow-up

## Usage

```
/migrate-service 42 BPA-jamaica BPA-lesotho2 --dry-run
/migrate-service 42 BPA-jamaica BPA-lesotho2
/migrate-service 17 BPA-elsalvador-dev BPA-investkenya
```
