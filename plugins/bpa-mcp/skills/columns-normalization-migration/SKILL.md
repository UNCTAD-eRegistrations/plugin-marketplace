   ...
   **stdin** and prints JSON on **stdout**:
   ...
   - **Over-12 split:**
     ...
     - **`action:"review"`** — a row the scan flags but never transforms
       ...
       feed it to the split apply. Two distinct reasons, with different
       dispositions:
       ...
       - **`in_grid_nested`** — an over-12 row nested inside an
         editgrid/datagrid but NOT as its direct child (e.g. a panel inside
         the grid). These render through the ordinary fluid flex model — the
         DS grid rule only reaches direct children — so the raggedness is
         real, but the APPLY for grid-nested rows is **DEFERRED to
         TOBE-18041** (grid-subtree insertion and editgrid submission-data
         survival are unproven). Record for a human / the inventory; do not
         feed to the split apply.

     In-grid classification (TOBE-18037): a row whose schema parent IS the
     editgrid/datagrid is a **direct child** — the DS grid rule lays it out
     correctly by construction, so both scanners skip it permanently (pad
     skip reason `grid_direct_child`; the split scan never emits it). A row
     nested deeper is an ordinary flex row: the pad scan reports it as skip
     `inside_grid_nested` (with its raw width sum, for the inventory) and
     the split scan surfaces it as the `in_grid_nested` review row above.
...
| `inside_grid_nested` | Nested deeper inside a grid — an ordinary flex row, so the raggedness is real, but the apply is **DEFERRED to TOBE-18041**. Carries a raw width sum for the inventory. |
...
- `null_width` / `invalid_width` only fail loud for a row that reaches the
  ...
  grid surfaces as a grid skip carrying a sum over its *valid* widths — not as
  malformed.
