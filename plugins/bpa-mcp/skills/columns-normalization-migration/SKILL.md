...
2. Run the form JSON through **both** scanners. Each reads the form JSON on
   **stdin** and prints JSON on **stdout**:
   ...
   - **Over-12 split:**
     ...
     `in_grid_nested` (with its raw width sum, for the inventory) and the
     split scan surfaces it as the `in_grid_nested` review row above — one
     concept, one spelling, on both tracks.
