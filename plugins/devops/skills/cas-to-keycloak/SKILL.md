...
3. **Credentials** go in `migrator-src/.env.lomas` — gitignored together with every `.env.lomas*` variant, never the tracked `.env`: `AUTH_URL`, `AUTH_REALM_NAME`, `AUTH_ADMIN_CLIENT`, `AUTH_ADMIN_SECRET`. With `AUTH_ADMIN_SECRET` set the script uses `client_credentials` against the **instance realm** — a confidential client whose service account holds `realm-management` → `realm-admin`; without it, the classic username/password flow. No `migrate-wrapper.js` realm swap is needed.
   A rehearsal realm uses its own file via **`--creds-file <path>`** (e.g. `.env.lomas.test`). An explicitly passed file must exist and must itself define all four `AUTH_*` keys, and its values override any ambient `AUTH_*` in the shell — so aiming at a rehearsal realm can never silently resolve to production. The flag is deliberately **not** named `--env-file`: Node claims that name even after the script path — it loads the file itself, and aborts before the script runs when the path is wrong. When the path exists Node loads it *and* still forwards the argument, so the script rejects `--env-file` explicitly instead of half-honouring it. A realm-scoped client cannot create realms, so create a rehearsal realm by importing a sanitised partial-export of the production realm in the admin console.
4. **Validate → pilot → import**: `npm run migrate-lomas -- --csv lomas-users.csv [--json lomas-users.json] --dry-run`, then import a handful of team-owned rows and walk forgot-password → login end-to-end, then the full run. A real import prints the target realm, URL and client and requires the realm name typed back before it writes (`--confirm-realm <realm>` when running non-interactively); `--dry-run` never constructs the Keycloak client at all. Existing realm users count as `skipped-existing` (409), so re-running with a fresher export right before cutover is safe; the script re-authenticates every 4 minutes and, should a re-auth fail mid-run, stops and still writes the audit report covering everything already created.
...
6. **Shared emails decide the fate of real people — choose the policy deliberately** with `--dup-policy`. Rows that are the same person listed twice (names equal ignoring case, accents and spacing) are always merged silently. When one email is shared by *differently named* people, the realm's one-account-per-email rule forces a choice: `exclude` (default) imports **none** of them and lists every one as `conflict-email` for the counterpart to resolve; `oldest` / `newest` keep the account with the earliest / latest creation date and list the rest as `duplicate`. Same-person merging happens before the date comparison, so an enriched JSON row is never dropped in favour of a bare CSV one, and a row whose date cannot be parsed never wins the comparison.
...
`legacy_created_at` is stamped in ISO form (`2019-03-07`). Tooling before 1.4.0 wrote the export's raw `d/m/yyyy`, and idempotence means existing accounts are never rewritten — so if any import ever ran against the realm on an older version, that cohort keeps the old format and a later query on the attribute silently misses it. Check before relying on it (`migrated_from=elomas-v2` with a non-ISO `legacy_created_at`) and normalise once if such accounts exist.
