...
3. **Credentials** go in `migrator-src/.env.lomas` — gitignored together with every `.env.lomas*` variant, never the tracked `.env`: `AUTH_URL`, `AUTH_REALM_NAME`, `AUTH_ADMIN_CLIENT`, `AUTH_ADMIN_SECRET`. With `AUTH_ADMIN_SECRET` set the script uses `client_credentials` against the **instance realm** — a confidential client whose service account holds `realm-management` → `realm-admin`; without it, the classic username/password flow. No `migrate-wrapper.js` realm swap is needed.
   A rehearsal realm uses its own file via **`--creds-file <path>`** (e.g. `.env.lomas.test`). An explicitly passed file must exist and must itself define all four `AUTH_*` keys, and its values override any ambient `AUTH_*` in the shell — so aiming at a rehearsal realm can never silently resolve to production. The flag is deliberately **not** named `--env-file`: Node claims that name, loads the file itself and aborts with its own error before the script ever sees the argument. A realm-scoped client cannot create realms, so create a rehearsal realm by importing a sanitised partial-export of the production realm in the admin console.
